// v8unpack #Область include_BlocklyExecutor_base_Variables

// BSLLS:UnusedLocalVariable-off
// Область переменных
//DynamicDirective
Перем BlocklyExecutor, context, Report, Workspace, размыватель, variables, ИдентификаторСессии, contexts, workspaces, ДлитОперации;
//DynamicDirective
Перем Ini, Operation, Executor, Functions, Step, Selected, Currentblock_context, ПараметрыРобота;
//DynamicDirective
Перем Proxy;
//DynamicDirective
// BSLLS:ExportVariables-off Экпспортные переменные для передачи между формами
Перем async_responces Экспорт, async_requests Экспорт;
// BSLLS:ExportVariables-on
Перем ИнтеграцияСтатусыДокументов,МенеджерыКриптографии;
//DynamicDirective
Перем ТранспортИнтеграции, КомпонентаИнтеграции, ОбработкаМенеджерыКриптографии, ДанныеОВерсииИнтеграции;
// BSLLS:UnusedLocalVariable-on
#КонецОбласти

#Область include_BlocklyExecutor_base_VariableSABYHTTPClient
#КонецОбласти

&НаКлиенте
Перем ГлобальныйКэш Экспорт;

&НаКлиенте
Перем СбисРаботаСJSON, ГлавноеОкно;

#Область include_core2_vo2_Модуль_Обработчики_Загрузка
#КонецОбласти

#Область include_core2_vo2_Модуль_МодульОбъектаКлиент
#КонецОбласти

#Область include_core2_vo2_Модуль_МодульОбъектаСервер
#КонецОбласти

#Область include_core2_vo2_Печать_Команда
#КонецОбласти

#Область include_core2_vo2_Модуль_МодульОбъектаСервер_vo3
#КонецОбласти
