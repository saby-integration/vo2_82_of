...
Процедура УстановитьВидимостьЭлементовВформеПросмотра(ФормаПросмотра, СоставПакета, КэшПарам) Экспорт
	...
	Иначе
		МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ФормаПросмотра, "Контент.Загрузка").Видимость = Ложь;	
	КонецЕсли;
	МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ФормаПросмотра, "Контент.Прохождение").Видимость = СоставПакета.Свойство("Событие");
