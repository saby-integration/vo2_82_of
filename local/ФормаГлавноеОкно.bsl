...
Процедура ОбновитьПоляФормы(СтруктураДляОбновленияФормы, ДопПараметры = Неопределено) Экспорт		
	...
	Если МодульОбъектаСервер().ЭтоТипСбис(СтруктураДляОбновленияФормы, "СбисИсключение") Тогда
		Возврат ;
	КонецЕсли;
...
Процедура ОтметитьВсеПриИзменении(Элемент)
	...
	КонецЦикла;
	ОтметитьВсе = Элемент.Значение;
	
	ИнициализацияРаботыВыделенияСтрок();
	ИзменитьСтроки(МассивСтрок, ОтметитьВсе);
	ИзменитьОтмеченФлажком(МассивСтрок);
...
Функция ОбновитьКонтент(АргументВходящие = Неопределено, ДопПараметры = Неопределено) Экспорт
	...
	Если МодульОбъектаКлиент().ЭтоТипСбис(СтруктураДляОбновленияФормы, "СбисИсключение") Тогда
		МодульОбъектаКлиент().СообщитьСбисИсключение(СтруктураДляОбновленияФормы);
		Возврат Истина;
	КонецЕсли;
