...
Функция ОбновитьКонтент(ЛокальныйКэш) Экспорт
	...
	Если ЗначениеЗаполнено(ГлавноеОкно.КонтрагентНоменклатуры) Тогда
		...
		стрФильтр.НаименованиеСБИС	= ЭтаФорма.ВладелецФормы.ФильтрМаска;
		стрФильтр.Номенклатура		= ЭтаФорма.ВладелецФормы.ФильтрНоменклатура1С;
		стрФильтр.ИдСБИС			= ЭтаФорма.ВладелецФормы.ФильтрКодКонтрагента;
		стрФильтр.GTIN				= ЭтаФорма.ВладелецФормы.ФильтрGTIN; 
		
		стрНавигация = Новый Структура("Страница, РазмерСтраницы");
		 
		стрНавигация.Страница		= ЭтаФорма.ВладелецФормы.ФильтрСтраница; //На апи отсчет страниц с 0, если на дбф с 1, перенести -1 в форму апи		
		стрНавигация.РазмерСтраницы = ЭтаФорма.ВладелецФормы.ЗаписейНаСтранице;
