...
Функция ОбновитьКонтентПодговитьРаздел(ЛокальныйКэш) Экспорт

	НастроитьКолонки(ЛокальныйКэш);
	НавигацияУстановитьПанель(ЛокальныйКэш);
	сбисЭлементФормы(ЛокальныйКэш.ГлавноеОкно,"ПанельФильтра").Видимость = Истина;
	сбисЭлементФормы(ЛокальныйКэш.ГлавноеОкно,"ПоказатьПанельФильтра").Видимость = Истина;
...
Процедура ФильтрОчистить(Кэш) Экспорт
	ГлавноеОкно = Кэш.ГлавноеОкно;
	Если Кэш.ТипыПолейФильтра.Свойство("ФильтрНоменклатура1С") Тогда
		ГлавноеОкно.ФильтрНоменклатура1С = Кэш.ТипыПолейФильтра.ФильтрНоменклатура1С.ПривестиЗначение();
	Иначе	
